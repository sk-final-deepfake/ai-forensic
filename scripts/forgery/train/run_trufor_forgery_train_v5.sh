#!/usr/bin/env bash
# ForenShield TruFor recipe v5 — v4 cache + infer-proxy BEST_KEY (cause #1 only)
...
#
# Default: reuse v4 prepare cache (SKIP_PREPARE=1). Same 400 train videos as v4.
...
EXP_NAME="${EXP_NAME:-forgery-v5-smoke-$(date +%Y%m%d-%H%M)}"
GPU="${GPU:-0}"
...
if [[ "$SKIP_PREPARE" != "1" ]]; then
  echo "[1/5] prepare frames (recipe v4 cache layout; same as v4)"
...
echo "[3/5] train (v5: BEST_KEY=tampered_pixel_recall, batch=$BATCH_SIZE, gpu=$GPU)"
...
echo ""
echo "=== v5 gate (mvtb @${INFER_THRESHOLD}) — compare to v4 @0.5 and v4 sweep @0.368 ==="
