#!/usr/bin/env bash
# Baseline-line R4 — cause #5 only (END_EPOCH increase)
#
# vs baseline infer: same models/test checkpoint → fine-tune (longer) → infer @0.5
# vs r0 (3ep): ONLY TRAIN.END_EPOCH differs in yaml (default 8)
# Independent from R1/R2/R3/Legacy (separate cache / run_name / r4 yaml)
# Data: default prepare — NO oversample, NO skip-OOW
#
# Usage (from ~/forenShield-ai/forgery):
#   sed -i 's/\r$//' scripts/train/run_trufor_forgery_train_r4.sh
#   bash scripts/train/run_trufor_forgery_train_r4.sh
#
# Optional: END_EPOCH=10 bash scripts/train/run_trufor_forgery_train_r4.sh
...
CACHE_ROOT="${ROOT}/data/processed/trufor-gmflow-train-400-r4"
EXP_NAME="${EXP_NAME:-forgery-r4-$(date +%Y%m%d-%H%M)}"
GPU="${GPU:-0}"
CONFIG_EXP="${CONFIG_EXP:-trufor_forgery_video_r4}"
...
if [[ "$SKIP_PREPARE" != "1" ]]; then
  echo "[1/5] prepare frames (R4: baseline-line default prepare; no oversample)"
...
echo "[2/5] ensure vendor dataset + config (Baseline-line r4 HP, END_EPOCH=${END_EPOCH})"
...
CFG_SRC="scripts/train/vendor_patches/trufor_forgery_video_r4.yaml"
CFG_DST="vendor/TruFor/TruFor_train_test/lib/config/trufor_forgery_video_r4.yaml"
cp "$CFG_SRC" "$CFG_DST"
echo "copied $CFG_DST"

echo "[3/5] train R4 (cause #5 epoch=${END_EPOCH}; batch=$BATCH_SIZE, gpu=$GPU)"
...
echo ""
echo "=== R4 gate @${INFER_THRESHOLD} (baseline only; vs R1/R2/R3 separate) ==="
...
echo ""
echo "done: $EXP_NAME"
echo "cache: $CACHE_ROOT (default prepare, no oversample)"
echo "config: $CONFIG_EXP END_EPOCH=${END_EPOCH}"
