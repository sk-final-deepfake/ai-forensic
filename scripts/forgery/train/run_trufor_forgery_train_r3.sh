...
# Independent from R1/R2/Legacy: baseline pretrain + v2 cache layout + oversample only.
#
# NOT stacked on R2: hard-negative OOW frames are KEPT (same as R1 / Legacy v2 prepare).
# Only change vs baseline smoke path: train_list positive lines x3.
#
...
GPU="${GPU:-0}"
CONFIG_EXP="${CONFIG_EXP:-trufor_forgery_video_r3}"
...
if [[ "$SKIP_PREPARE" != "1" ]]; then
  echo "[1/5] prepare frames (R3: v2 layout + positive oversample x${OVERSAMPLE_POSITIVE}; NO skip-OOW)"
...
CFG_SRC="scripts/train/vendor_patches/trufor_forgery_video_r3.yaml"
CFG_DST="vendor/TruFor/TruFor_train_test/lib/config/trufor_forgery_video_r3.yaml"
cp "$CFG_SRC" "$CFG_DST"
echo "copied $CFG_DST"

echo "[3/5] train R3 (cause #2 oversample only; BEST_KEY=avg_p-F1, batch=$BATCH_SIZE, gpu=$GPU)"
...
echo ""
echo "=== R3 gate @${INFER_THRESHOLD} (baseline only; independent from R1/R2) ==="
...
echo ""
echo "done: $EXP_NAME"
echo "cache: $CACHE_ROOT (v2 layout, oversample x${OVERSAMPLE_POSITIVE})"
